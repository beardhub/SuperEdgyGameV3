function KeysFramework(){
	...
	this.setupListeners = function(keyhub){window.addEventListener("keydown",(function(c){this.get(c.keyCode,true).down=true;}).bind(this));
	window.addEventListener("keyup",(function(c){this.get(c.keyCode,true).down=false;}).bind(this));
	for(var i=0;i<3;i++)window.addEventListener("key"+(["down","up","press"])[i],keyhub.dup.bind(keyhub,this,i));}
